	...
	Vue.component('respec-button', {
		...
		template: `
			<div v-if="tmp[layer].buyables && tmp[layer].buyables.respec && !(tmp[layer].buyables.showRespec !== undefined && tmp[layer].buyables.showRespec == false)">
				<div class="tooltipBox respecCheckbox"><input type="checkbox" v-model="player[layer].noRespecConfirm" ><tooltip v-bind:text="'禁用重分配确认'"></tooltip></div>
				<button v-on:click="respecBuyables(layer)" v-bind:class="{ longUpg: true, can: player[layer].unlocked, locked: !player[layer].unlocked }" style="margin-right: 18px">{{tmp[layer].buyables.respecText ? tmp[layer].buyables.respecText : "重分配"}}</button>
			</div>
			`
	})
			...
			<table>
				<tr v-for="a in hasMilestone('s',2) ? (hasMilestone('s',3) ? 5 : 4) : 3">
					<td style="border: 4px solid #ffffff; height: 125px; min-width: 175px; max-width: 175px;">
						你拥有 {{format(data.s.stringTypes[a])}} {{layers.s.getStringName(a)}} 字符串，将 {{layers.s.getStringName(a-1)}} 字符串的获取速度乘以 {{format(layers.s.stringTypeEffect(a))}}倍。<br>({{format(layers.s.stringTypeGen(a))}}/秒)
					</td>
					<td v-for="b in hasMilestone('s',0) ? 4 : 3" style="border: 4px solid #ffffff; height: 125px; min-width: 175px; max-width: 175px;">
						{{b == 1 ? "长度" : (b == 2 ? "宽度" : (b == 3 ? "深度" : "测试"))}}: {{formatWhole(player.s.dimensionUpgrades[(4*(a-1)) + b])}}米
