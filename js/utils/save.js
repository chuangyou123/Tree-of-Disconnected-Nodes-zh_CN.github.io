...
function fixData(defaultData, newData) {
	for (item in defaultData) {
		...
		else if (defaultData[item] instanceof Decimal) { // 转换为Decimal
			if (newData[item] === undefined)
				newData[item] = defaultData[item];
	...
	for (item in data) {
		if (data[item] == null) {
			...
			if (!NaNalert) {
				...
				NaNalert = true;
				alert("在玩家数据中发现无效值，名为 '" + item + "'。请告知这个模组的创建者！你可以刷新页面，数据将被修复。")
				return
			}
		}
...
function importSave(imported = undefined, forced = false) {
	...
	try {
		...
		if (tempPlr.versionType != getModID() && !forced && !confirm("这个存档似乎来自另一个模组！你确定要导入吗？")) // 错误的存档（使用"Forced"来强制接受。）
			return;
		player = tempPlr;
